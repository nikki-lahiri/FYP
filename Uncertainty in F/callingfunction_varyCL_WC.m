...
for i=1:sz_omega
    ...
    [w_clas,risk_clas,ret_clas,sharpe_clas]= classical_minvar(n_assets,p,V,F,f,asset_ret,alpha);
    %[w,risk,ret,sharpe]= classical_minvar(n_assets,mu,asset_ret,alpha);
    %clas_policy=w
%     ret_vec(i)=ret;
    %--------robust optimisation---------
    [w_rob,risk_rob,ret_rob,sharpe_rob,mu0,gam,rho,D_bar,w_coef,eig_H,eta]= robust2(n_assets, m_factors,p,F,f,D,asset_ret,omega_vec(i),alpha);
    
    %-------worst case classical---------------
%     %equally weighted worst case
%     w_clas=(1/n_assets)*ones(n_assets,1);
    [risk_wc,ret_wc,sharpe_wc]=wc_classical2(n_assets,m_factors,w_clas,rho,w_coef,eig_H,D_bar,mu0,gam,eta);
