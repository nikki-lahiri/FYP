function [risk_classical_wc,wc_ret,sharpe_wc]=wc_classical2(n_assets,m_factors,classical_weights,rho,w_coef,eig_H,D_bar,mu0,gam,eta)
...
%D1_3
A4=[2*rho',zeros(1,m_factors),0,0,0;
  zeros(1,n_assets),zeros(1,m_factors), 1,-1,0];
b4=[0;0];
...
tmp_t=eye(m_factors);
for i=1:m_factors
    A4= [2*w_coef(m_factors+1-i,:),zeros(1,m_factors),0,0,0;
   zeros(1,n_assets),-tmp_t(:,m_factors+1-i)', -eig_H(m_factors+1-i),0,0];
    b4=[0;-1];
...
Aineq4=[zeros(1,n_assets),ones(1,m_factors),0,1,-(1-eta);%d1
   zeros(1,n_assets),zeros(1,m_factors),1,0,0]; %d2
bineq4=[0;%d1
...
risk_classical_wc=fval4+classical_weights'*D_bar*classical_weights;
